    ...
    if( statements ) {
        ...
        statements.reverse().map( (statement, statementIndex) => {
            ...
            if( dataType === 'string' && globalOnConflict === 'union' && val ) {
                ...
                // results === null has incomplete parameters, this is to enforce the parameter subsets rule
                if( results !== null || (statementIndex === statements.length - 1 && !formula) ){
                    if( results !== null ) this.errors = null;
