...
const itemContains = (item, subItem) => {
    ...
    return Object.keys(subItem).every( filterField => {
        const filterValues = subItem[ filterField ];
...
const itemDoesntContain = (item, subItem) => {
    if( Object.keys(subItem).length === 0 ) return false;
    return Object.keys(subItem).every( filterField => {
        const filterValues = subItem[ filterField ];
   ...
   exceptions
    ...
    const applyFilter = () => {
        ...
        Object.keys(filter).map( filterType => { //if(!filters[filterType]) return error;
            if( [ "sort", "size" ].includes( filterType ) ){
                results = filters[ filterType ]( results, filter[ filterType ] )
            } else {
                results = results.filter( item => {
                    if( !uniqueIds.includes( item.id ) && filters[filterType](item, filter[filterType]) ){
