...
module.exports = (function( ){
    ...
    if( this.appliedConditionIds && this.appliedConditionIds.length > 0 ) joinAppliedConditions( this.appliedConditionIds );
    ...
    if( this.ruleDependencyIds && this.parameterIds ){
        this.ruleDependencyIds.map( ruleId => {
            if( this.parameterIds.includes( ruleId ) ){
                ...
                if( !ruleDependency ) throw new Error( `Rule Dependency not found, RuleId: ${ruleId} `);
