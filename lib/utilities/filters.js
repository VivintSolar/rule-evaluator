    ...
    const sortMethods = {
        priorityList: ( field, pList ) => {
            return sortedList.sort( ( itemA, itemB ) => {
                ...
                if( !pList.includes( itemB[field] ) && pList.includes( itemA[field] ) ) return -1;
                return pList.indexOf( itemB[field] ) < pList.indexOf( itemA[field] ) ? 1 : -1;
            })
        },
        alphaNumeric: field => {
            const sortAlphaNumeric = (itemA, itemB) => {
                const reA = /[^a-zA-Z]/g;
                const reN = /[^0-9]/g;
                const aA = itemA[field].replace(reA, "");
                const bA = itemB[field].replace(reA, "");
                if (aA === bA) {
                    const aN = parseInt(itemA[field].replace(reN, ""), 10);
                    const bN = parseInt(itemB[field].replace(reN, ""), 10);
                    return aN === bN ? 0 : aN > bN ? 1 : -1;
                } else {
                    return aA > bA ? 1 : -1;
                }
            }
            const sortAlphaNum = (itemA, itemB) => itemA[field].localeCompare(itemB[field], 'en', { numeric: true })
            return sortedList.sort( sortAlphaNum )
        }
    }
