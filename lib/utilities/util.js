...
const sorter = {
    alphabetical: ( template, collection, targetField ) => {
        ...
        return collection
            .sort((a, b) =>  a[targetField].toLowerCase().localeCompare( b[targetField].toLowerCase() ))
            .sort( (a,b)=> {
                const aNumber = parseFloat(a[targetField].replace( /^\D+/g, '') || "0" );
                const bNumber = parseFloat(b[targetField].replace( /^\D+/g, '') || "0" );
